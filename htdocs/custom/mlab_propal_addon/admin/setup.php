...
/**
 * \file    mlab_propal_addon/admin/setup.php
 * \ingroup mlab_propal_addon
 * \brief   Mlab_propal_addon setup page.
 */
...
require_once '../lib/mlab_propal_addon.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "mlab_propal_addon@mlab_propal_addon"));
...
$arrayofparameters = array(
	'MLAB_PROPAL_ADDON_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'MLAB_PROPAL_ADDON_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/mlab_propal_addon/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'MLAB_PROPAL_ADDON_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'MLAB_PROPAL_ADDON_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "Mlab_propal_addonSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_mlab_propal_addon@mlab_propal_addon');

// Configuration header
$head = mlab_propal_addonAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "mlab_propal_addon@mlab_propal_addon");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("Mlab_propal_addonSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'MLAB_PROPAL_ADDON_'.strtoupper($myTmpObjectKey).'_ADDON';
