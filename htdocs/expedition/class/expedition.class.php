...
class Expedition extends CommonObject {
				...
				for ( $i = 0; $i < $num; $i ++ ) {
					if ( empty( $this->lines[ $i ]->product_type ) || ! empty( $conf->global->STOCK_SUPPORTS_SERVICES ) ) {
						if ( ! isset( $this->lines[ $i ]->detail_batch ) ) {    // no batch management
							if ( $this->create_line( $this->lines[ $i ]->entrepot_id,
							                         $this->lines[ $i ]->origin_line_id,
							                         $this->lines[ $i ]->qty,
							                         $this->lines[ $i ]->rang,
							                         $this->lines[ $i ]->array_options ) <= 0 ) {
								$error ++;
							}
						} else {    // with batch management
							if ( $this->create_line_batch( $this->lines[ $i ],
							                               $this->lines[ $i ]->array_options ) <= 0 ) {
								$error ++;
							}
						}
	...
	public function create_line_batch( $line_ext, $array_options = 0 ) {
		...
		foreach ( $stockLocationQty as $stockLocation => $qty ) {
			$line_id = $this->create_line( $stockLocation,
			                               $line_ext->origin_line_id,
			                               $qty,
			                               $line_ext->rang,
			                               $array_options );
			if ( $line_id < 0 ) {
			...
			if ( preg_match( '/^[\(]?PROV/i', $this->ref ) ) {
				...
				if ( ! $error && file_exists( $dirsource ) ) {
					...
					if ( @rename( $dirsource, $dirdest ) ) {
						...
						$listoffiles = dol_dir_list( $conf->expedition->dir_output . '/sending/' . $newref,
						                             'files',
						                             1,
						                             '^' . preg_quote( $oldref, '/' ) );
				...
				if ( $value['q'] > 0 || ( $value['q'] == 0 && getDolGlobalString( 'SHIPMENT_GETS_ALL_ORDER_PRODUCTS' ) ) ) {
					...
					if ( getDolGlobalString( "STOCK_MUST_BE_ENOUGH_FOR_SHIPMENT", '0' ) ) {
						...
						if ( $prod_batch->qty < $linebatch->qty ) {
							$langs->load( "errors" );
							$this->errors[] = $langs->trans( 'ErrorStockIsNotEnoughToAddProductOnShipment',
							                                 $prod_batch->fk_product );
							dol_syslog( get_class( $this ) . "::addline_batch error=Product " . $prod_batch->batch . ": " . $this->errorsToString(),
							            LOG_ERR );
							$this->db->rollback();
			...
			if ( $resql ) {
				...
				for ( $i = 0; $i < $cpt; $i ++ ) {
					...
					if ( empty( $lotArray ) ) {
						...
						$result = $mouvS->reception( $user,
						                             $obj->fk_product,
						                             $obj->fk_entrepot,
						                             $obj->qty,
						                             0,
						                             $langs->trans( "ShipmentCanceledInDolibarr",
						                                            $this->ref ) ); // Price is set to 0, because we don't want to see WAP changed
						if ( $result < 0 ) {
							$error ++;
						...
						foreach ( $lotArray as $lot ) {
							$result = $mouvS->reception( $user,
							                             ...
							                             $lot->qty,
							                             0,
							                             $langs->trans( "ShipmentCanceledInDolibarr", $this->ref ),
							                             $lot->eatby,
							                             $lot->sellby,
							                             $lot->batch ); // Price is set to 0, because we don't want to see WAP changed
							if ( $result < 0 ) {
								$error ++;
								$this->errors = array_merge( $this->errors, $mouvS->errors );
								break;
				...
				for ( $i = 0; $i < $cpt; $i ++ ) {
					...
					if ( empty( $lotArray ) ) {
						...
						$result = $mouvS->reception( $user,
						                             ...
						                             $obj->qty,
						                             0,
						                             $langs->trans( "ShipmentDeletedInDolibarr",
						                                            $this->ref ) ); // Price is set to 0, because we don't want to see WAP changed
						if ( $result < 0 ) {
							$error ++;
							$this->errors = array_merge( $this->errors, $mouvS->errors );
						...
						foreach ( $lotArray as $lot ) {
							$result = $mouvS->reception( $user,
							                             ...
							                             $lot->qty,
							                             0,
							                             $langs->trans( "ShipmentDeletedInDolibarr", $this->ref ),
							                             $lot->eatby,
							                             $lot->sellby,
							                             $lot->batch ); // Price is set to 0, because we don't want to see WAP changed
							if ( $result < 0 ) {
		...
		if ( $resql ) {
			...
			while ( $i < $num ) {
				...
				$localtax_array = array(
					0 => $obj->localtax1_type,
					1 => $obj->localtax1_tx,
					2 => $obj->localtax2_type,
					3 => $obj->localtax2_tx
				);
				...
				// For invoicing
				$tabprice              = calcul_price_total( $obj->qty_shipped,
				                                             $obj->subprice,
				                                             $obj->remise_percent,
				                                             $obj->tva_tx,
				                                             $localtax1_tx,
				                                             ...
				                                             $obj->fk_product_type,
				                                             $mysoc,
				                                             $localtax_array ); // We force type to 0
	...
	public function getTooltipContentArray( $params ) {
		...
		$datas          = array();
		$datas['picto'] = img_picto( '',
		                             $this->picto ) . ' <u class="paddingrightonly">' . $langs->trans( "Shipment" ) . '</u>';
		if ( isset( $this->statut ) ) {
		...
		if ( ! $nofetch ) {
			...
			$datas['customer'] = '<br><b>' . $langs->trans( 'Customer' ) . ':</b> ' . $this->thirdparty->getNomUrl( 1,
			                                                                                                        '',
			                                                                                                        0,
			                                                                                                        1 );
		}
	...
	public function getNomUrl(
		...
		$result .= $linkstart;
		if ( $withpicto ) {
			$result .= img_object( ( $notooltip ? '' : $label ),
				( $this->picto ? $this->picto : 'generic' ),
				( $notooltip ? ( ( $withpicto != 2 ) ? 'class="paddingright"' : '' ) : 'class="' . ( ( $withpicto != 2 ) ? 'paddingright ' : '' ) . '"' ),
				0,
				0,
				$notooltip ? 0 : 1 );
		}
		...
		$reshook    = $hookmanager->executeHooks( 'getNomUrl',
		                                          $parameters,
		                                          $this,
	...
	public function getUrlTrackingStatus( $value = '' ) {
		...
		if ( ! empty( $tracking ) && ! empty( $value ) ) {
			...
			$this->tracking_url = sprintf( '<a target="_blank" rel="noopener noreferrer" href="%s">' . ( $value ? $value : 'url' ) . '</a>',
			                               $url,
			                               $url );
		} else {
					...
					for ( $i = 0; $i < $cpt; $i ++ ) {
						...
						if ( empty( $obj->edbrowid ) ) {
							...
							$result = $mouvS->livraison( $user,
							                             ...
							                             - $qty,
							                             $obj->subprice,
							                             $langs->trans( "ShipmentUnClassifyCloseddInDolibarr",
							                                            $this->ref ) );
							if ( $result < 0 ) {
								$this->error  = $mouvS->error;
								$this->errors = $mouvS->errors;
	...
	public function generateDocument(
		...
		return $this->commonGenerateDocument( $modelpath,
		                                      $modele,
		                                      $outputlangs,
		                                      $hidedetails,
		                                      $hidedesc,
	...
	public function update( $user = null, $notrigger = 0 ) {
		...
		if ( ! empty( $batch ) && isModEnabled( 'productbatch' ) ) {
			...
			if ( empty( $batch_id ) || empty( $this->fk_product ) ) {
				dol_syslog( get_class( $this ) . '::update missing fk_origin_stock (batch_id) and/or fk_product',
				            LOG_ERR );
				$this->errors[] = 'ErrorMandatoryParametersNotProvided';
				$error ++;
			}
