 ...
 */
class pdf_arch_filters extends ModelePDFPropales {
	/**
	...
	public function write_file( $object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0 ) {
		...
		$this->param_preventivo = $object->array_options;

		$this->atleastonediscount = ( 1 == $param_preventivo['options_sconto_stampa'] ) ? 1 : 0;

		// Se lo sconto non è stampato imposta il delta di spostamento delle colonne precedenti e allarga la descrizione
		$delta_sconto = ( 2 == $this->param_preventivo['options_sconto_stampa'] )
			? 15
			: 0;
		...
		if ( $conf->propal->multidir_output[ $conf->entity ] ) {
			...
			if ( $object->specimen ) {
				...
				$file      = $dir . "/" . $objectref . "-filtrato.pdf";
			}
			...
			if ( file_exists( $dir ) ) {
				...
				if ( empty( $this->atleastonediscount ) & 1 == 2 ) {
				...
				// Loop on each lines
				for ( $i = 0; $i < $nblines; $i ++ ) {
					// Estrai i dati extra della riga
					$line_data = [
						'product_type' => $object->lines[ $i ]->product_type,
						'total_ht'     => $object->lines[ $i ]->total_ht,
					];

					foreach ( $object->lines[ $i ]->array_options as $key => $item ) {
						$line_data[ $key ] = $item;
					}

					// Se è un blocco imposto i dati per le righe successive
					if ( 99 == $line_data['product_type'] ) {

						$labelproductservice = pdf_getlinedesc(
							$object,
							$i,
							$outputlangs,
							0,
						...
						$block_settings  = [
							'mostra_intestazione' => $line_data['options_mostra_intestazione'],
							'mostra_dettagli'     => $line_data['options_mostra_dettagli'],
							'mostra_prezzi'       => $line_data['options_mostra_prezzi'],
							'mostra_totale'       => $line_data['options_mostra_totale'],
							'ricarico_extra'      => $line_data['options_ricarico_extra'],
							'forza_totale'        => $line_data['options_forza_totale'],
							'conteggio_righe'     => $line_data['options_conteggio_righe'],
							'totale_blocco'       => $line_data['options_totale_netto'],
							'titolo_blocco'       => $labelproductservice,
						];
						$relative_lin_nr = 0;
						$max_lines_nr    = $line_data['options_conteggio_righe'];
					} else {
						// Altrimenti incremento il contatore delle righe
						$relative_lin_nr ++;
					}
...
//					}
					...
					pdf_writelinedesc(
						...
						$curX,
						$curY,
						$block_settings['mostra_dettagli'],
						$block_settings['mostra_intestazione']
					);
...
//					}
					...
					// Se è un Blocco non stampare le altre colonne
					if ( 99 != $line_data['product_type'] ) {
						// Quantity
						$qty = pdf_getlineqty( $object, $i, $outputlangs, $hidedetails ) . ' nr.';
						$pdf->SetXY( $this->posxdesc + $this->posxqty + $delta_sconto, $curY );
						$pdf->MultiCell( 20 - 0.8, 4, $qty, 0, 'R' ); // Enough for 6 chars

						// Unit price before discount
						$up_excl_tax = pdf_getlineupexcltax( $object, $i, $outputlangs, $hidedetails ) . ' €';
						$pdf->SetXY( $this->posxdesc + $this->posxup + $delta_sconto, $curY );
						$pdf->MultiCell( 25 - 0.8, 3, $up_excl_tax, 0, 'R', 0 );

						// Stampa lo sconto solo se è impostato da visualizzare
						if ( 1 == $this->param_preventivo['options_sconto_stampa'] ) {
							$pdf->SetXY( $this->posxdesc + $this->posxdiscount, $curY );
...
//						$remise_percent = pdf_getlineremisepercent($object, $i, $outputlangs, $hidedetails);
//						$pdf->MultiCell(20 + 2, 3, $remise_percent, 0, 'R');
//					}
						...
						$pdf->SetXY( $this->posxdesc + $this->postotalht, $curY );
						$pdf->MultiCell( 24, 3, $total_excl_tax, 0, 'R', 0 );

					} elseif ( 99 == $line_data['product_type'] && 1 == $block_settings['mostra_intestazione'] ) {
						// riempi le colonne solo con lo sfondo
						$font_size = $pdf->getFontSize( $parameters['outputlangs'] ) * 4;
						$pdf->SetFontSize( $font_size );

						// Quantity
						$pdf->SetXY( $this->posxdesc + $this->posxqty, $curY );
						$pdf->MultiCell( 90 - 1, 3, '', 0, 'R', true ); // Enough for 6 chars

					}

					// Check if we are at the end of a block if so print block total
					if ( $relative_lin_nr == $max_lines_nr && 1 == $block_settings['mostra_totale'] ) {
						...
						$block_total = sprintf(
							'Totale %1$s: %2$s €',
							$block_settings['titolo_blocco'],
							number_format(
								$block_settings['totale_blocco'],
								2,
								',',
								'.'
							)
...
//						$pdf->SetLineStyle(array('dash'=>0));
					...
					while ( $pagenb < $pageposafter ) {
						...
						if ( $pagenb == 1 ) {
							...
							$this->_tableau( $pdf, $tab_top_newpage - 7, $this->page_hauteur - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 1, 1, $object->multicurrency_code );
						}
				...
				if ( $pagenb == 1 ) {
					...
					$this->_tableau( $pdf, $tab_top_newpage - 7, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code );
					$bottomlasttab = $this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforsignature - $heightforfooter + 1;
					//$bottomlasttab = 50;
				}
...
//		}
		...
		$pdf->MultiCell( 140 + $delta_sconto, 2, $outputlangs->transnoentities( "Designation" ), 0, 'L' );
...
//		}
		...
		// Separatore colonna
		//$pdf->line( $this->posxdesc + $this->posxqty, $tab_top, $this->posxdesc + $this->posxqty, $tab_top + $tab_height );

		$pdf->SetXY( $this->posxdesc + $this->posxqty + $delta_sconto, $tab_top + 1 );
		$pdf->MultiCell( 20, 2, $outputlangs->transnoentities( "Q.tà" ), 0, 'R' );

		// Separatore colonna
		//$pdf->line( $this->posxdesc + $this->posxup, $tab_top, $this->posxdesc + $this->posxup, $tab_top + $tab_height);

		$pdf->SetXY( $this->posxdesc + $this->posxup + $delta_sconto, $tab_top + 1 );
		$pdf->MultiCell( 25, 2, $outputlangs->transnoentities( "Prezzo" ), 0, 'R' );

		// Mostra o meno la colonna dello sconto
		if ( 1 == $this->param_preventivo['options_sconto_stampa'] ) {

			// Separatore colonna
			//$pdf->line( $this->posxdesc + $this->posxdiscount, $tab_top, $this->posxdesc + $this->posxdiscount, $tab_top + $tab_height);

			$pdf->SetXY( $this->posxdesc + $this->posxdiscount, $tab_top + 1 );
			$pdf->MultiCell( 15, 2, $outputlangs->transnoentities( "Sconto" ), 0, 'C' );

		}

		// Separatore colonna
		//$pdf->line( $this->posxdesc + $this->postotalht, $tab_top, $this->posxdesc + $this->postotalht, $tab_top + $tab_height );

		$pdf->SetXY( $this->posxdesc + $this->postotalht, $tab_top + 1 );
		$pdf->MultiCell( 24, 2, $outputlangs->transnoentities( "Totale" ), '', 'R' );
	}
